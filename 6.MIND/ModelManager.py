    ...
    def train_step(self, ds,epoch,summary_writer):
        ...
        def train_loop(inputs):
            with tf.GradientTape() as tape:
                ...
                labels = tf.concat([tf.ones((output.shape[0], 1)), tf.zeros((output.shape[0], output.shape[1] - 1))],
                                   axis=1)
                loss_value = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=output)

                loss_value = tf.reduce_sum(loss_value) + l2_reg
                gradients = tape.gradient(loss_value, self.model.trainable_variables)
                self.opt.apply_gradients(zip(gradients, self.model.trainable_variables))

                self.metric_loss.update_state(loss_value)
                self.metric_auc.update_state(labels, logits["output"])
