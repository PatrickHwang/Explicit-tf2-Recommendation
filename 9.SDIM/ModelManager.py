...
class ModelManager:
    ...
    def make_layer_choice(self,layer_name='mind_layer',model_params={}):
        if layer_name=='mind_layer':
            self.layer=MINDLayer(feature_names=self.categorical_features,feature_dims=150000,embedding_dims=self.embedding_dims)
    ...
    def train_step(self, ds,epoch,summary_writer):
        ...
        def train_loop(inputs):
            with tf.GradientTape() as tape:
                result = self.model(inputs, training=True)
                gsu_logits,esu_logits=result['gsu_logits'],result['esu_logits']

                labels = inputs.pop(self.label_name)
                gsu_loss = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=gsu_logits)
                esu_loss = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=esu_logits)
                alpha=0.2
                beta=0.8
                total_loss=alpha*gsu_loss+beta*esu_loss

                loss_value = tf.reduce_sum(total_loss)
                gradients = tape.gradient(loss_value, self.model.trainable_variables)
                self.opt.apply_gradients(zip(gradients, self.model.trainable_variables))

                self.metric_loss.update_state(loss_value)

                esu_positive_prob = esu_logits[:, 1]  # Extract the positive class probability
                true_labels = tf.argmax(labels, axis=1)  # Convert one-hot labels to binary labels
                self.metric_auc.update_state(true_labels, esu_positive_prob)
