...
class ModelManager:
    ...
    def __init__(self,feature_names=['user_tag1', 'user_tag2','item_tag1','item_tag2','item_tag3'],
                 json_path='data/generated/data_info.json', embedding_dims=16, lr=0.00003, label_name='label',
                 checkpoint_dir='retrieval_model/checkpoint',tensorboard_dir='retrieval_model/tensorboard',
                 data_dir='data/generated-non-time-sequence', output_dir='retrieval_model/output', n_threads=-1, shuffle=50, batch=100, epochs=30,
                 restore=False, num_trials=3,layer='fm_ranking',checkpoint_interval=True,model_params={}):
        # 模型参数和存储路径
        ...
        self.load_json_info(json_path)
        self.feature_dims=self.feature_info[-1]
    ...
    def set_feature_names(self, feature_names=None, label_name=None):
        if feature_names:
    ...
    def make_layer_choice(self,layer_name='fm_ranking',model_params={}):
        if layer_name=='fm_ranking':
            self.layer=FMRankingLayer(feature_names=self.feature_names,feature_dims=self.feature_dims,embedding_dims=self.embedding_dims,**self.model_params)
        elif layer_name=='deepfm_ranking':
            self.layer=DeepFMRankingLayer(feature_names=self.feature_names,feature_dims=self.feature_dims,embedding_dims=self.embedding_dims,**self.model_params)
        elif layer_name=='wide_and_deep_ranking':
            self.layer=WideAndDeepRankingLayer(feature_names=self.feature_names,feature_dims=self.feature_dims,embedding_dims=self.embedding_dims,**self.model_params)
        elif layer_name=='dssm_single_tower':
            self.layer=DSSMSingleTowerLayer(**self.model_params)
        elif layer_name=='dssm_double_tower':
            if len(self.model_params)==0:
                self.layer=DSSMTwoTowerRetrievalLayer(u_feature_names=['user_tag1','user_tag2'],i_feature_names=['item_tag1','item_tag2','item_tag3'],
                                                      u_feature_dims=self.feature_dims,i_feature_dims=self.feature_dims)
            else:
                self.layer=DSSMTwoTowerRetrievalLayer(**self.model_params)
        elif layer_name=='ffm_ranking':
            self.layer=FFMRankingLayer(feature_names=self.feature_names,feature_dims=self.feature_dims,embedding_dims=self.embedding_dims,**self.model_params)
        elif layer_name=='pnn_ranking':
            method=model_params.get('method','inner')
            kernel_type=model_params.get('kernel_type','mat')
            self.layer = PNNRankingLayer(feature_names=self.feature_names, feature_dims=self.feature_dims,
                                         embedding_dims=self.embedding_dims, method=method,kernel_type=kernel_type)
        else:
        ...
        while True:
            ...
            if step % 500 == 0:
                ...
                with summary_writer.as_default():  # 指定记录器
                    tf.summary.scalar("train_auc", result['auc'], step=(epoch*6000+step))
                    tf.summary.scalar("train_loss", result['loss'], step=(epoch*6000+step))
        result = train_loop_end()
        print(f"train over-{step} : {result}")
        return result
...
if __name__ == '__main__':
    ...
    """
    mm = ModelManager(feature_names=eval(args.feature_names), json_path=args.json_path, embedding_dims=args.embedding_dims,lr=args.lr,
                   label_name=args.label_name, checkpoint_dir='ranking_model/checkpoint',
                   data_dir=args.data_dir, output_dir='ranking_model/output', n_threads=args.n_threads, shuffle=args.shuffle,
                   batch=args.batch, epochs=args.epochs, restore=args.restore, num_trials=args.num_trials,
                      layer='deepfm_ranking',checkpoint_interval=args.checkpoint_interval,
                      tensorboard_dir='ranking_model/tensorboard',
                      model_params=eval(args.model_params))
    """
    """
